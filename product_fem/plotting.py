...
def plot(func):
    ...
    if is_control:
        axs = control_plot(func)
        ax = tuple(axs)
    elif is_function and len(func) < 3:
        ax = fenics_plot(func)
        ax.figure = fig
    elif is_function and len(func) == 3:
        ax = covariance_plot(func)
        ax.figure = fig

    return fig, ax

def ellipse_patch(cov, x, y, ax):
...
def plot_ellipse_field(sig, ax):
    ...
    for i, c in enumerate(cov):
        ...
        ax.add_patch(ellipse_patch(c, x, y, ax))
    return ax